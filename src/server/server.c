...
int perform_key_exchange(int sockfd, unsigned char *session_key) {
  ...
  if (crypto_kx_server_session_keys(rx_key, tx_key, server_public_key, server_secret_key, client_public_key) != 0) {
...
/* Encrypt message */
int encrypt_message(const unsigned char *plaintext, size_t plaintext_len, unsigned char *ciphertext,
                    unsigned long long nonce_counter, const unsigned char *key) {
  unsigned char nonce[crypto_aead_chacha20poly1305_IETF_NPUBBYTES] = {0};
  memcpy(nonce, &nonce_counter, sizeof(nonce_counter));

  unsigned long long ciphertext_len;
  int result = crypto_aead_chacha20poly1305_ietf_encrypt(ciphertext, &ciphertext_len, plaintext, plaintext_len, NULL, 0,
                                                         NULL, nonce, key);
...
/* Decrypt message */
int decrypt_message(const unsigned char *ciphertext, size_t ciphertext_len, unsigned char *plaintext,
                    unsigned long long nonce_counter, const unsigned char *key) {
  unsigned char nonce[crypto_aead_chacha20poly1305_IETF_NPUBBYTES] = {0};
  memcpy(nonce, &nonce_counter, sizeof(nonce_counter));

  unsigned long long plaintext_len;
  int result = crypto_aead_chacha20poly1305_ietf_decrypt(plaintext, &plaintext_len, NULL, ciphertext, ciphertext_len,
                                                         NULL, 0, nonce, key);
...
void get_client_ip(struct sockaddr_in addr, char *buffer) {
  sprintf(buffer, "%d.%d.%d.%d", addr.sin_addr.s_addr & 0xff, (addr.sin_addr.s_addr & 0xff00) >> 8,
          (addr.sin_addr.s_addr & 0xff0000) >> 16, (addr.sin_addr.s_addr & 0xff000000) >> 24);
}
...
void send_message_encrypted(char *plaintext, int sender_uid) {
  ...
  for (int i = 0; i < MAX_CLIENTS; ++i) {
    if (clients[i]) {
      ...
      int encrypted_len =
          encrypt_message((unsigned char *)plaintext, plaintext_len, encrypted + sizeof(unsigned long long),
                          clients[i]->tx_nonce_counter, clients[i]->session_key);
      ...
      if (write(clients[i]->sockfd, encrypted, total_len) < 0) { perror("ERROR: write to descriptor failed"); }
    }
  }
  pthread_mutex_unlock(&clients_mutex);
}
...
void *handle_client(void *arg) {
  ...
  if (name_recv <= sizeof(unsigned long long)) {
    ...
    int decrypted_len =
        decrypt_message(encrypted_buff + sizeof(unsigned long long), name_recv - sizeof(unsigned long long),
                        decrypted_buff, received_nonce, cli->session_key);
  ...
  while (1) {
    ...
    if (receive > sizeof(unsigned long long)) {
      ...
      int decrypted_len =
          decrypt_message(encrypted_buff + sizeof(unsigned long long), receive - sizeof(unsigned long long),
                          decrypted_buff, received_nonce, cli->session_key);
...
int main(int argc, char **argv) {
  ...
  if (load_or_generate_server_keypair() < 0) { return EXIT_FAILURE; }
