...
int perform_key_exchange(int sockfd, unsigned char *session_key) {
  ...
  if (crypto_kx_client_session_keys(rx_key, tx_key, client_public_key, client_secret_key, server_public_key) != 0) {
...
void send_msg_handler() {
  ...
  while (1) {
    ...
    if (strncmp(message, "/exit", 5) == 0) {
      ...
      int encrypted_len = encrypt_message((unsigned char *)buffer, strlen(buffer),
                                          encrypted + sizeof(unsigned long long), send_nonce_counter);
...
void recv_msg_handler() {
  ...
  while (1) {
    ...
    if (receive > sizeof(unsigned long long)) {
      ...
      int decrypted_len = decrypt_message(encrypted + sizeof(unsigned long long), receive - sizeof(unsigned long long),
                                          decrypted, received_nonce);
...
int main(int argc, char **argv) {
  ...
  int encrypted_len = encrypt_message((unsigned char *)name, strlen(name), encrypted_name + sizeof(unsigned long long),
                                      send_nonce_counter);
